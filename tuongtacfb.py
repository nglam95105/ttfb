...
def banner():
    ...
    print(f'''
           ...
           {trang}╚══██╔══╝╚══██╔══╝╚══██╔══╝██╔═══██╗██╔═══██╗██║ 
            ...
            {xanh}   ██║      ██║      ██║   ╚██████╔╝╚██████╔╝███████╗ 
            {trang}  ╚═╝      ╚═╝      ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝ ''')
    print(Colorate.Horizontal(Colors.white_to_blue,"         © Bản Quyền ThùyTrang ! Tool Gộp Siêu Lỏ !!!"))
    thanhngang(65)
    print(f'''{thanh}{luc}Admin{trang}: {vang}Nguyễn thùy TrangTrang
{thanh}{luc}Box Zalo{trang}: {do}https://zalo.me/g/rqmeyv489
{thanh}{luc}ib ad thu xu{trang}: {do}09728822150972882215
...
def Delay(value):
    while not(value <= 1):
        value -= 0.123
        print(f'''{trang}[{xanh}ThuỳTrang{trang}] [{xanh}DELAY{trang}] [{xanh}{str(value)[0:5]}{trang}] [{vang}X    {trang}]''', '               ', end = '\r')
        sleep(0.02)
        print(f'''{trang}[{xanh}ThuỳTrang{trang}] [{xanh}DELAY{trang}] [{xanh}{str(value)[0:5]}{trang}] [ {vang}X   {trang}]''', '               ', end = '\r')
        sleep(0.02)
        print(f'''{trang}[{xanh}ThuỳTrang{trang}] [{xanh}DELAY{trang}] [{xanh}{str(value)[0:5]}{trang}] [  {vang}X  {trang}]''', '               ', end = '\r')
